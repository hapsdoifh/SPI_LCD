...
static void print_usage(const char *prog) {
    ...
    puts("  -D --device   device to use (default /dev/spidev1.1)\n"
            ...
            "  -i --changeimg change displayed image\n");
    exit(1);
}


LCD_Image displayImg = {
	.m_width = 320,
	.m_hight = 240,
    .m_image = plant_img,
};
...
static void parse_opts(int argc, char *argv[]) {
    while (1) {
        ...
        c = getopt_long(argc, argv, "D:F:s:d:b:g:l:iHOLC3NR", lopts, NULL);
        ...
        switch (c) {
            ...
            case 'i': 
                printf("argv:%.1s",argv[2]);
                displayImg.m_image = imgList[(int)(argv[2][0])-48]; 
                break;
...
int main(int argc, char *argv[]) {
    ...
    if (ret >= 0) {
        if (Spi_Is_From_File()) {
            ...
            Lcd_ili9341_ShowIcon(static_fd_lcd, 0, 0, 320, 240, displayImg.m_image);
